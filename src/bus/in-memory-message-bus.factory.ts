...
export class InMemoryMessageBusFactory implements IMessageBusFactory {
  ...
  constructor(
      ...
      @Inject(Service.MIDDLEWARE_REGISTRY) private middlewareRegistry: MiddlewareRegistry,
  ) {
  }

  create(channel: Channel): IMessageBus {
    if (!(channel instanceof InMemoryChannel)) {
      throw new Error('Only InMemoryChannel is supported');
    }

    return new InMemoryMessageBus(this.registry, this.middlewareRegistry, channel);
  }
}
