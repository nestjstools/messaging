...
export class InMemoryMessageBus implements IMessageBus {
  ...
  async dispatch(message: RoutingMessage): Promise<object | void> {
    const middlewares = [];
    middlewares.push(...this.channel.config?.middlewares ?? [], ...message.messageOptions?.middlewares ?? []);
    ...
    for (const handler of handlers) {
        for (const middlewareClass of middlewares) {
          const middleware = this.middlewareRegistry.getByName(DecoratorExtractor.extractMessageMiddleware(middlewareClass));
