...
import { MESSAGE_HANDLER_METADATA, MESSAGING_MIDDLEWARE_METADATA } from './decorator';
...
export const registerMiddlewares = (moduleRef: ModuleRef, discoveryService: DiscoveryService) => {
  const registry: MiddlewareRegistry = moduleRef.get(Service.MIDDLEWARE_REGISTRY);
  const logger: MessagingLogger = moduleRef.get(Service.LOGGER);
  const middlewareInstances = discoveryService
      ...
      .filter(middleware => {
        ...
        return Reflect.hasMetadata(MESSAGING_MIDDLEWARE_METADATA, middleware.metatype)
      });

  middlewareInstances.forEach(middleware => {
    logger.log(`Middleware [${middleware.constructor.name}] was registered`);
    //@ts-ignore
    registry.register(Reflect.getMetadata(MESSAGING_MIDDLEWARE_METADATA, middleware.metatype), middleware.instance);
  })
}
